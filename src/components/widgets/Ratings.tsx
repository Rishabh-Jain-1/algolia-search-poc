...
const Ratings = ({ currentRefinement, refine, createURL, count }: any) => {
  const ratings = new Array(4).fill(null).map((_, ratingIndex) => {
    const value = 4 - ratingIndex;

    const itemsCount = count
      .filter((countObj: any) => value <= parseInt(countObj.value, 10))
      .map((countObj: any) => countObj.count)
      .reduce((sum: any, currentCount: any) => sum + currentCount, 0);
